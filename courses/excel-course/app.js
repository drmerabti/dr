(function () {
  ...
  const LESSONS = [
    { id: 1, icon: '🖥️', title: 'التعرف على برنامج Excel وواجهة البرنامج' },
    { id: 2, icon: '💾', title: 'إنشاء ملفات Excel وفتحها وحفظها' },
    { id: 3, icon: '✍️', title: 'إدخال البيانات وتعديلها داخل الخلايا' },
    { id: 4, icon: '📋', title: 'تحديد الخلايا والصفوف والأعمدة وإدارتها' },
    { id: 5, icon: '🎨', title: 'تنسيق الخلايا والخطوط والألوان' },
    { id: 6, icon: '🔢', title: 'تنسيق الأرقام والتواريخ والنسب المئوية' },
    { id: 7, icon: '📑', title: 'النسخ واللصق والتعبئة التلقائية AutoFill' },
    { id: 8, icon: '🔍', title: 'البحث والاستبدال وترتيب البيانات' },
    { id: 9, icon: '⌨️', title: 'أهم اختصارات لوحة المفاتيح في Excel' },
    { id: 10, icon: '🧊', title: 'تجميد الأجزاء Freeze Panes' },
    { id: 11, icon: '📊', title: 'إنشاء الجداول وتنسيقها' },
    { id: 12, icon: '➕', title: 'العمليات الحسابية الأساسية في Excel' },
    { id: 13, icon: '🔗', title: 'فهم المراجع النسبية والمطلقة والمختلطة' },
    { id: 14, icon: 'Σ', title: 'الدوال الأساسية: SUM وAVERAGE وMIN وMAX' },
    { id: 15, icon: '❓', title: 'الدوال الشرطية IF وAND وOR' },
    { id: 16, icon: '🔢', title: 'دوال العد COUNT وCOUNTA وCOUNTIF' },
    { id: 17, icon: '🔤', title: 'دوال النصوص ومعالجة البيانات النصية' },
    { id: 18, icon: '📅', title: 'دوال التاريخ والوقت' },
    { id: 19, icon: '🔎', title: 'البحث عن البيانات باستخدام VLOOKUP وXLOOKUP' },
    { id: 20, icon: '🔽', title: 'تصفية البيانات باستخدام Filter' },
    { id: 21, icon: '↕️', title: 'الفرز المتقدم للبيانات باستخدام Sort' },
    { id: 22, icon: '🎯', title: 'التنسيق الشرطي Conditional Formatting' },
    { id: 23, icon: '📈', title: 'إنشاء المخططات والرسوم البيانية' },
    { id: 24, icon: '🖨️', title: 'إعداد الصفحة والطباعة في Excel' },
    { id: 25, icon: '🔒', title: 'حماية الخلايا وملفات Excel' },
    { id: 26, icon: '✅', title: 'التحقق من صحة البيانات والقوائم المنسدلة' },
    { id: 27, icon: '🧹', title: 'التعامل مع البيانات الكبيرة وتنظيفها' },
    { id: 28, icon: '🗂️', title: 'الجداول المحورية Pivot Tables' },
    { id: 29, icon: '📊', title: 'تحليل البيانات باستخدام Pivot Charts' },
    { id: 30, icon: '🔄', title: 'استيراد وتصدير البيانات وربط الجداول' },
    { id: 31, icon: '🏆', title: 'مشروع تطبيقي شامل: إنشاء ملف Excel احترافي من البداية إلى النهاية' },
  ];
  // No lesson has a built memory game yet for this course — all show "قريبًا" until content is ready.
  const MEMORY_GAME_URLS = {};
  ...
  function notebookLocalKey(lessonId) {
    ...
    return `excelCourseNotebook:${user ? user.uid : 'anon'}:${lessonId}`;
  }
  ...
  $('notebookSaveBtn').addEventListener('click', async () => {
    ...
    try {
      await firebase.firestore()
        ...
        .collection('excelCourseNotebooks').doc('lesson-' + currentLessonId)
  ...
  function loadChat(lessonId) {
    ...
    const ref = firebase.firestore()
      .collection('users').doc(user.uid)
      .collection('excelCourseQuestions').doc('lesson-' + lessonId)
  ...
  $('chatSendBtn').addEventListener('click', async () => {
    ...
    await firebase.firestore()
      .collection('users').doc(user.uid)
      .collection('excelCourseQuestions').doc('lesson-' + currentLessonId)
  ...
  async function hasCourseAccess(uid) {
    ...
    try {
      const doc = await firebase.firestore().collection('users').doc(uid).get();
      const sub = doc.exists ? doc.data().excelCourseSubscription : null;
  ...
  els.subscribeBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      const res = await fn({ product: 'excelCourse' });
